...
class ICON_NWP():
    ...
    def load_icon_data(self,file_name):
        """
    ...
    def load_and_upsample_icon(self,var):
        var="Hydrometeor"#
        #Open first hour
        ...
        for icon_h in self.icon_var_list:
            ...
            icon_ds=icon_h.load_icon_data(file_name)
        ...
        icon_upsampled_hmp={}
        icon_upsampled_hmp["IWV"]=iwv_icon.resample(time="1min").\
                                        interpolate("linear")
        icon_upsampled_hmp["LWP"]=lwp_icon.resample(time="1min").\
                                        interpolate("linear")
        icon_upsampled_hmp["IWP"]=iwp_icon.resample(time="1min").\
                                        interpolate("linear")
        self.icon_upsampled_hmp=icon_upsampled_hmp
    
    @staticmethod
    def lookup_ICON_AR_period_data(flight,ar_of_day,resolution,
